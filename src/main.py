...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, mode, rela_2sec_matrix, ent_2sec_matrix,):
    ...
    model.eval()
    # Use the most recent snapshots from history_list
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        # Build historical entity and relationship graphs
        history_glist = [build_sub_graph(num_nodes, num_rels, g, use_cuda, args.gpu) for g in input_list]
        history_rel_g_list = [utils.build_relation_sub_graph(num_rels, snap, use_cuda, args.gpu) for snap in input_list]

        # Generate inverse triples
        inverse_triples = test_snap[:, [2, 1, 0]]
        inverse_triples[:, 1] = inverse_triples[:, 1] + num_rels
        que_pair = e2r(test_snap, num_rels)
        que_pair_inv = e2r(inverse_triples, num_rels)

        # Get current entity subgraphs
        sub_snap, sub_snap_inv = get_sample_from_history_graph3(subg_arr, sr_to_sro, test_snap, num_nodes, num_rels, use_cuda, args.gpu)

        # 构建关系图（结合历史和当前）
        combined_triples = np.concatenate([np.concatenate(history_list[-args.test_history_len:]), test_snap])
        # rel_subg_snap = utils.build_relation_sub_graph(num_rels, combined_triples, use_cuda, args.gpu)
        combined_inv_triples = np.concatenate([np.concatenate([snap[:, [2, 1, 0]] for snap in history_list[-args.test_history_len:]]), inverse_triples])
        ...
        rel_subg_snap = None
        rel_subg_snap_inv = None

        # Predict with relationship graphs
        test_triples, final_score = model.predict(
            que_pair, sub_snap, time_idx, history_glist, num_rels, static_graph,
            test_triples_input, use_cuda, rela_2sec_matrix, ent_2sec_matrix, history_rel_g_list, rel_subg_snap
        )
        inv_test_triples, inv_final_score = model.predict(
            que_pair_inv, sub_snap_inv, time_idx, history_glist, num_rels, static_graph,
            test_triples_input_inv, use_cuda, rela_2sec_matrix, ent_2sec_matrix, history_rel_g_list, rel_subg_snap_inv
        )

        # Calculate ranks and MRR
        ...
        # Update input_list for the next iteration
        if args.multi_step:
    ...
    # ranks_raw = torch.cat(ranks_raw) if ranks_raw else torch.tensor([], device='cuda:0')
    # print("ranks_raw after concat:", ranks_raw)
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    rela_concept_mat = f'./data/{args.dataset}/auxiliary/rel2sec_matrix_v1.npy'
    ent_concept_mat = f'./data/{args.dataset}/auxiliary/ent2sec_matrix_v1.npy'

    rela_2sec_matrix = torch.Tensor(np.load(rela_concept_mat))
    ent_2sec_matrix = torch.Tensor(np.load(ent_concept_mat))


    if use_cuda:
        rela_2sec_matrix = rela_2sec_matrix.cuda()
        ent_2sec_matrix = ent_2sec_matrix.cuda()
    ...
    model = RecurrentRGCN(args.decoder,
                        ...
                        use_cuda=use_cuda,
                        gpu = args.gpu,
                        analysis=args.run_analysis, rela_2sec_matrix=rela_2sec_matrix, ent_2sec_matrix=ent_2sec_matrix)
    ...
    # if args.test :
        ...
        mrr_filter= test(model,
                                ...
                                model_state_file, 
                                static_graph, 
                                "test", rela_2sec_matrix=rela_2sec_matrix, ent_2sec_matrix=ent_2sec_matrix)
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                subgraph_arr = np.load(f'./data/{args.dataset}/his_graph_for/train_s_r_{train_sample_num}.npy')
                subgraph_arr_inv = np.load(f'./data/{args.dataset}/his_graph_inv/train_o_r_{train_sample_num}.npy')
                ...
                for id in range(2): 
                    if id %2 ==0: 
                        loss_e, loss_r, loss_static, loss_cl = model.get_loss(que_pair, subg_snap, train_sample_num, history_glist, triples, static_graph, use_cuda, rela_2sec_matrix, ent_2sec_matrix, history_rel_g_list, rel_subg_snap)
                    else:
                        loss_e, loss_r, loss_static, loss_cl = model.get_loss(que_pair_inv, subg_snap_inv, train_sample_num, history_glist, inverse_triples, static_graph, use_cuda, rela_2sec_matrix, ent_2sec_matrix, history_rel_g_list, rel_subg_snap_inv)
            ...
            if epoch and epoch % args.evaluate_every == 0:
                mrr_filter = test(model, 
                                    ...
                                    model_state_file, 
                                    static_graph, 
                                    mode="train", rela_2sec_matrix=rela_2sec_matrix, ent_2sec_matrix=ent_2sec_matrix)
        ...
        mrr_filter = test(model, 
                            ...
                            model_state_file, 
                            static_graph, 
                            mode="test", rela_2sec_matrix=rela_2sec_matrix, ent_2sec_matrix=ent_2sec_matrix)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--n-epochs", type=int, default=333,
