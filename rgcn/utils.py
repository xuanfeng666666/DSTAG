...
def build_relation_graph(num_rels, triples, use_cuda, gpu):
    """
    Optimized version of build_relation_graph.
    
    :param num_rels: Total number of relation types.
    :param triples: Triples data in format [src, rel, dst, fre].
    :param use_cuda: Boolean indicating whether to use CUDA.
    :param gpu: GPU device ID.
    :return: A DGL graph representing the relation graph.
    """
    def comp_deg_norm(g):
    ...
    # 获取唯一关系类型作为节点
    unique_rels = np.unique(rel)
    num_nodes = len(unique_rels)
    # num_nodes = max(max(src), max(dst)) + 1  # 确保节点数足够大
    rel_to_idx = {r: i for i, r in enumerate(unique_rels)}

    # 使用 defaultdict(set) 记录每个实体关联的关系
    src_rels = defaultdict(set)
    dst_rels = defaultdict(set)
    triples = triples[:, :3]  # Add this line
    ...
    edges_dst = []
    edge_fre = []
    for entity_rels in [src_rels, dst_rels]:
        for rel_set in entity_rels.values():
            rel_list = list(rel_set)
            for i in range(len(rel_list)):
                for j in range(i + 1, len(rel_list)):
                    r1, r2 = rel_list[i], rel_list[j]
                    if r1 != r2:
                        edges_src.append(rel_to_idx[r1])
                        edges_dst.append(rel_to_idx[r2])
                        edge_fre.append(1)  # 频率可根据需求调整

    # 去除重复边并累加频率
    edge_dict = defaultdict(int)
    for s, d, f in zip(edges_src, edges_dst, edge_fre):
        edge_dict[(s, d)] += f
    edges_src, edges_dst, edge_fre = zip(*[(s, d, f) for (s, d), f in edge_dict.items()])
    ...
    orig_rel_id = torch.LongTensor(unique_rels).view(-1, 1)
    g.ndata.update({'id': node_id, 'orig_id': orig_rel_id, 'norm': norm.view(-1, 1)})

    # 设置边属性
    if len(edges_src) > 0:
        g.edata['fre'] = torch.LongTensor(edge_fre)
...
def build_relation_sub_graph(num_rels, triples, use_cuda, gpu):
    """
    Build a relation subgraph where nodes are relation types and edges represent co-occurrence.
    
    :param num_rels: Total number of relation types.
    :param triples: Triples data in format [src, rel, dst].
    :param use_cuda: Boolean indicating whether to use CUDA.
    :param gpu: GPU device ID.
    :return: A DGL graph representing the relation subgraph.
    """
    # 计算归一化因子
    def comp_deg_norm(g):
        in_deg = g.in_degrees(range(g.number_of_nodes())).float()
        in_deg[torch.nonzero(in_deg == 0).view(-1)] = 1  # 避免除以 0
        norm = 1.0 / in_deg
        return norm
    ...
    # 获取唯一的关系类型（如果 triples 中的关系 ID 不连续）
    unique_rels = np.unique(rel)
    rel_to_idx = {r: i for i, r in enumerate(unique_rels)}  # 关系 ID 到新索引的映射

    # 构建共现边
    cooccur_dict = defaultdict(set)
    for s, r, o in triples:
        r_idx = rel_to_idx[r]
        # 检查与 s 相连的其他关系
        for r2 in np.unique(rel[src == s]):
            if r != r2:
                cooccur_dict[r_idx].add(rel_to_idx[r2])
        # 检查与 o 相连的其他关系
        for r2 in np.unique(rel[dst == o]):
            if r != r2:
                cooccur_dict[r_idx].add(rel_to_idx[r2])
    ...
    edges_src = []
    edges_dst = []
    for r1, r2_set in cooccur_dict.items():
        for r2 in r2_set:
            edges_src.append(r1)
            edges_dst.append(r2)
    ...
    # 创建关系图
    g = dgl.DGLGraph()
    g.add_nodes(len(unique_rels))  # 节点数为唯一关系类型数
    ...
    # 设置节点属性
    node_id = torch.arange(0, len(unique_rels), dtype=torch.long).view(-1, 1)
    orig_rel_id = torch.LongTensor(unique_rels).view(-1, 1)  # 原始关系 ID
    g.ndata.update({
        'id': node_id,
        'orig_id': orig_rel_id,
        'norm': norm.view(-1, 1)
...
def stat_ranks(rank_list, method):
    hits = [1, 3, 10]
    # print('----------------------------')
    # print('rank_list len:', len(rank_list))

    # print('rank_list:', rank_list)
    # print('rank_list shape:', rank_list.shape)
